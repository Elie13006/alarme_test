...
let expName = 'detection_alarme';  // from the Builder filename that created this script
let expInfo = {
...
function welcomeRoutineEachFrame() {
  return async function () {
    ...
    if (key_resp_welcome.status === PsychoJS.Status.STARTED) {
      let theseKeys = key_resp_welcome.getKeys({keyList: ['n'], waitRelease: false});
...
function trialRoutineEachFrame() {
  return async function () {
    ...
    if (key_resp.status === PsychoJS.Status.STARTED) {
      let theseKeys = key_resp.getKeys({keyList: ['n'], waitRelease: false});
  ...
  return async function () {
    ...
    for (let eachResp=0; eachResp<psychoJS.experiment._trialsData.length; eachResp++) {
        if (psychoJS.experiment._trialsData[eachResp]['thiscolor']== 'red.png') {
            ...
            nb_oran_vert += 1;
            if (psychoJS.experiment._trialsData[eachResp]['key_resp.corr']== 0){
            nInCorr += 1
            }
        }
    }
